...
│   ├── price_model.pkl             # Generated on Phase 3
│   └── knn_model.pkl               # Generated on Phase 3
├── notebooks/
...
└── src/
    ...
    │   └── dag_reviews.py
    ├── models/
    │   └── model_selection.py         # MLflow model comparison script and training
...
to the terminal and saved to `standalone_admin_password.txt` or `simple_auth_manager_passwords.json.generated`.
...
|-----|----------|-----------|
| `airbnb_master_pipeline` | listings.csv | Extract, Transform , Enrich, Validate, EDA report, save Gold parquet, Load to Kafka |
| `airbnb_calendar_pipeline` | calendar.csv | Extract, Transform, Enrich , Validate, EDA report, Load to Kafka |
| `airbnb_reviews_pipeline` | reviews.csv | Extract, Transform , Enrich, Validate, EDA report, Load to Kafka |
...
These queries read from the Kafka Gold topics produced by Phase 1.
...
Reads the `pipeline_errors` topic and displays all validation failures in
append mode.

### Query 4 — Geospatial sentiment map 
...
Make sure your virtual environment is active on this stage.
...
mlflow ui --port 5000
```
This evaluates multiple regression algorithms to predict listing prices, logging metrics to MLflow and retraining the winning model on 100% of the dataset. It also fits a Nearest Neighbors (KNN) model to power the live streaming recommendation system.
...
|-----|-------|-------------|
| Search & Map | Batch | Filters 20K+ listings; Map with clustering and satellite toggle; CSV export |
| Price Prediction | Streaming | Host form → Kafka → Spark  model → predicted price + neighbourhood benchmark |
